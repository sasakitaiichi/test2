...
public class ShoppingCartController {
    ...
    @GetMapping("/shop-cart-sort")
    public String cartListPageSort(@RequestParam Map<String, Object> params, HttpServletRequest request,
                                   HttpSession httpSession) {
        String order = null;
        String orderBy = null;
        NewBeeMallUserVO user = (NewBeeMallUserVO) httpSession.getAttribute(Constants.MALL_USER_SESSION_KEY);
        ...
        if (params.containsKey("order") && !StringUtils.isEmpty(params.get("order") + "")) {
            order = params.get("order") + "";
        }

        List<NewBeeMallShoppingCartItemVO> myShoppingCartItems = newBeeMallShoppingCartService.getMyShoppingCartItemsBySort(user.getUserId(), order, orderBy);
        if (!CollectionUtils.isEmpty(myShoppingCartItems)) {
            //购物项总数
            itemsTotal = myShoppingCartItems.stream().mapToInt(NewBeeMallShoppingCartItemVO::getGoodsCount).sum();
            if (itemsTotal < 1) {
    ...
    public Result cartListPageOrder(@RequestParam Map<String, Object> params, HttpServletRequest request,
        ...
        Map myShoppingCartItemsResult = new HashMap();
        List<NewBeeMallShoppingCartItemVO> myShoppingCartItems = newBeeMallShoppingCartService.getMyShoppingCartItemsBySort(user.getUserId(), order, orderBy);

            myShoppingCartItemsResult.put("myShoppingCartItems",myShoppingCartItems);

        return ResultGenerator.genSuccessResult(myShoppingCartItemsResult);
    }
