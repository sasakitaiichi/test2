...
public class NewBeeMallShoppingCartServiceImpl implements NewBeeMallShoppingCartService {
    ...
    @Override
    public List<NewBeeMallShoppingCartItemVO> getMyShoppingCartItemsBySort(Long newBeeMallUserId, String order, String orderBy) {
        List<NewBeeMallShoppingCartItem> newBeeMallShoppingCartItems = null;
        List<NewBeeMallShoppingCartItemVO> newBeeMallShoppingCartItemVOS = new ArrayList<>();
        if (!StringUtils.isEmpty(orderBy) && orderBy.equals("goodsCount")) {
            newBeeMallShoppingCartItems = newBeeMallShoppingCartItemMapper.selectByGoodsCount(newBeeMallUserId, Constants.SHOPPING_CART_ITEM_TOTAL_NUMBER, order, orderBy);
        } else {
            newBeeMallShoppingCartItems = newBeeMallShoppingCartItemMapper.selectByUserId(newBeeMallUserId, Constants.SHOPPING_CART_ITEM_TOTAL_NUMBER);
        }
        if (!CollectionUtils.isEmpty(newBeeMallShoppingCartItems)) {
            //查询商品信息并做数据转换
            List<Long> list = newBeeMallShoppingCartItems.stream().map(NewBeeMallShoppingCartItem::getGoodsId).collect(Collectors.toList());

            List<NewBeeMallGoods> newBeeMallGoods = newBeeMallGoodsMapper.selectBySellingPrice(list, order, orderBy);
            ...
            if (!StringUtils.isEmpty(orderBy) && orderBy.equals("sellingPrice")) {

                if (!StringUtils.isEmpty(order) && order.equals("asc")) {
                    newBeeMallShoppingCartItemVOS = newBeeMallShoppingCartItemVOS.stream().sorted(Comparator.comparingInt(NewBeeMallShoppingCartItemVO::getSellingPrice)).collect(Collectors.toList());
                }
                if (!StringUtils.isEmpty(order) && order.equals("desc")) {
                    newBeeMallShoppingCartItemVOS = newBeeMallShoppingCartItemVOS.stream().sorted(Comparator.comparingInt(NewBeeMallShoppingCartItemVO::getSellingPrice).reversed()).collect(Collectors.toList());
                }
            }
        }
        return newBeeMallShoppingCartItemVOS;
